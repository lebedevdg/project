...
### О чем этот проект
Реализован деплой бота Robot и вспомогательных приложений(RabbitMQ, mongodb etc), а также систем мониторинга, логирования и алертинга в облако GCP. Сборка и тестирование в Gitlab pipelines
...
### Описание
Данный проект рассчитан на запуск и работу в окружении GCP - Google Cloud Platform (https://cloud.google.com/). Так же для запуска проекта на машине, с которой будет осуществляться развертывание всех окружений (управляющая машина), должны быть установлены docker, docker-compose, docker-machine.

Т.к. сборка образов - это достаточно ресурсозатратный процесс и к тому же требующий быстрого интернета, весь процесс рекомендуется производить в контексте ранее созданной машины GCP уровнем не ниже n1-standard-1 
